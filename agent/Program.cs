...
internal sealed class LunaChatForm : Form
{
    ...
    public LunaChatForm(LunaCore core)
    {
        ...
        _brain = new BrainPanel
        {
            Left = 762, Top = 116, Width = 254, Height = 250,
        ...
        Shown += (_, _) =>
        {
            AppendLuna("Olá. Eu sou a LUNA IA.\n\nMeu núcleo local está ativo. Eu observo, interpreto, decido, executo e verifico. Esta versão também possui uma base local de conhecimento para preservar minha identidade e meu contexto de construção.");
            _input.Focus();
        };
    ...
    private async Task SubmitAsync()
    {
        ...
        _status.Text = "●  LUNA IA RACIOCINANDO";
        _status.ForeColor = Accent;
        _brainState.Text = "OBSERVANDO • INTERPRETANDO • DECIDINDO";
        _brain.Active = true;
        try
        {
            var result = await _core.ProcessAsync(text);
            AppendLuna(result.Text);
            _status.Text = result.Executed ? "●  AÇÃO VERIFICADA" : "●  LUNA IA ONLINE";
            _brainState.Text = result.Executed ? "EXECUÇÃO + VERIFICAÇÃO CONCLUÍDAS" : "AGUARDANDO PRÓXIMO OBJETIVO";
        }
        catch (Exception ex)
        {
            AppendLuna("Erro interno controlado: " + ex.Message);
            _status.Text = "●  ERRO CONTROLADO";
...
internal sealed class BrainLogo : Control
{
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        var r = new Rectangle(8, 9, Width - 16, Height - 18);
        e.Graphics.DrawEllipse(pen, r);
        using var node = new SolidBrush(Accent);
        e.Graphics.FillEllipse(node, Width / 2 - 4, Height / 2 - 4, 8, 8);
        e.Graphics.DrawArc(pen, 16, 18, 18, 18, 205, 220);
        e.Graphics.DrawArc(pen, Width - 34, 18, 18, 18, 115, 220);
        e.Graphics.DrawLine(pen, Width / 2, 14, Width / 2, Height - 14);
    }
}
...
internal sealed class BrainPanel : Panel
{
    ...
    protected override void OnPaint(PaintEventArgs e)
    {
        ...
        var cx = Width / 2;
        var cy = Height / 2;
        var radius = Math.Min(Width, Height) / 2 - 38;
        using var ring = new Pen(Color.FromArgb(35, 214, 255), 1.2F);
        e.Graphics.DrawEllipse(ring, cx - radius, cy - radius, radius * 2, radius * 2);
        e.Graphics.DrawEllipse(ring, cx - radius + 18, cy - radius + 18, (radius - 18) * 2, (radius - 18) * 2);
        var nodes = new[] { (cx, cy - radius + 6), (cx + radius - 6, cy), (cx, cy + radius - 6), (cx - radius + 6, cy), (cx + 45, cy - 38), (cx - 42, cy + 34) };
        using var line = new Pen(Color.FromArgb(45, 113, 91, 255), 1.1F);
        foreach (var n in nodes) e.Graphics.DrawLine(line, cx, cy, n.cx, n.cy);
        var glow = Active ? 8 + (Pulse % 10) : 5;
        using var core = new SolidBrush(Color.FromArgb(56, 214, 255));
        e.Graphics.FillEllipse(core, cx - glow, cy - glow, glow * 2, glow * 2);
        using var dot = new SolidBrush(Color.FromArgb(113, 91, 255));
        foreach (var n in nodes) e.Graphics.FillEllipse(dot, n.cx - 4, n.cy - 4, 8, 8);
        using var label = new SolidBrush(Color.FromArgb(133, 154, 184));
        using var font = new Font("Consolas", 8F, FontStyle.Bold);
        var text = Active ? "THINKING" : "READY";
        e.Graphics.DrawString(text, font, label, cx - 27, Height - 27);
    }
}
