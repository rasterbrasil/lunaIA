...
if (-not $ollamaCommand) {
    ...
    if ($possible) { $ollamaExe = $possible }
    else { throw "Ollama foi instalado, mas o executável não foi encontrado. Reinicie o Windows e execute este script novamente." }
} else { $ollamaExe = $ollamaCommand.Source }
...
# ------------------------------------------------
# 2. VOZ NEURAL LOCAL — Piper TTS pt-BR feminina
# ------------------------------------------------
...
$voiceModel = Join-Path $voicesDir "dii_pt-BR.onnx"
$voiceConfig = Join-Path $voicesDir "dii_pt-BR.onnx.json"
...
if (-not (Test-Path $voiceModel) -or -not (Test-Path $voiceConfig)) {
    Write-Host "Baixando voz neural feminina em português do Brasil (aprox. 64 MB)..." -ForegroundColor Cyan
    $voiceBase = "https://huggingface.co/OpenVoiceOS/pipertts_pt-BR_dii/resolve/main"
    Invoke-WebRequest -Uri "$voiceBase/dii_pt-BR.onnx" -OutFile $voiceModel
    Invoke-WebRequest -Uri "$voiceBase/dii_pt-BR.onnx.json" -OutFile $voiceConfig
}
...
Write-Host "Voz: Piper neural pt-BR feminina (Dii)"
Write-Host "Local dos recursos: $ttsRoot"
Write-Host ""
Write-Host "O modo 'Conversar por texto' funciona sem microfone." -ForegroundColor Green
