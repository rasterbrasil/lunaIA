...
class LunaAccessibilityService : AccessibilityService() {
    ...
    @Volatile var lastResult: String = "Pronto."
        private set
    ...
    override fun onServiceConnected() {
        ...
        val code = prefs.getString("pair_code", "") ?: ""
    ...
    private fun openApp(name: String): Boolean {
        val normalized = name.lowercase().replace("á", "a").replace("ã", "a").replace("ç", "c").trim()
        val packages = when {
            normalized.contains("chrome") -> listOf("com.android.chrome")
            normalized.contains("whatsapp") -> listOf("com.whatsapp")
            normalized.contains("youtube") -> listOf("com.google.android.youtube")
            normalized.contains("config") || normalized.contains("settings") -> listOf("com.android.settings")
            normalized.contains("calculadora") || normalized.contains("calculator") -> listOf("com.google.android.calculator", "com.sec.android.app.popupcalculator")
            normalized.contains("gmail") -> listOf("com.google.android.gm")
            else -> emptyList()
        }
        for (pkg in packages) {
            val intent = packageManager.getLaunchIntentForPackage(pkg)
            if (intent != null) { intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK); startActivity(intent); return true }
        }
        return false
    }
    ...
    private fun observe(): String {
        val root = rootInActiveWindow ?: return "nenhuma janela ativa"
        val parts = mutableListOf<String>()
        collectText(root, parts, 0)
        return parts.distinct().take(40).joinToString(" | ").ifBlank { "janela ativa sem texto acessível" }
    }
    ...
    private fun findNodeByText(node: AccessibilityNodeInfo?, text: String): AccessibilityNodeInfo? {
        ...
        if (node.text?.toString()?.contains(text, ignoreCase = true) == true || node.contentDescription?.toString()?.contains(text, ignoreCase = true) == true) return node
        for (i in 0 until node.childCount) findNodeByText(node.getChild(i), text)?.let { return it }
        return null
    }
}
